...
#     QStylePainter, QStyle,QHBoxLayout,QWidget,QLineEdit,QVBoxLayout,QLabel,QFormLayout,QCheckBox,QMainWindow,QMenu, QAction,QStatusBar
...
QLabel#label { font-size: 27pt;}
QLabel#predict_lable{ font-size: 27pt;} 
/*
QPushButton#xxx
或者
#xx
    ...
    def get_ser_device(self):
        ...
        try:
            self.ser = serial.Serial(
                self.ser_device, 9600, 8, "E", timeout=50, stopbits=1)
        except:
...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs) -> None:
        ...
        # self.ser = serial.Serial("COM5", 9600, 8, "E", timeout=50,stopbits=1)
    ...
    def init_model(self):
        ...
        model_input = self.model_ft.downsample_layers[0][0]
        ...
        self.model_ft.load_state_dict(torch.load(
            "./model_last_.pth", map_location=self.DEVICE))
        self.model_ft.eval()
        self.model_ft.to(self.DEVICE)
        # label_class=["母鸡","公鸡"]

    def out_serial(self):
        if self.predict_index == 2:
            return
        ...
        # 串口发送数据
        self.child.ser.write(k)
        time.sleep(2)
        self.child.ser.write(d)
    ...
    def predict(self, img):
        ...
        # img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        img = self.transform_test(Image.fromarray(img))
        img.unsqueeze_(0)
        img = Variable(img).to(self.DEVICE)
        out = self.model_ft(img)
        _, pred = torch.max(out.data, 1)
        self.predict_index = pred.data.item()
        pred_label = self.classes[self.predict_index]
        confident = out
        conf = float(confident[0][self.predict_index]*100)
        print("predice:{},conf:{}".format(pred_label, conf))
        end_time=time.time()
    ...
    def initUI(self):
        ...
        # self.predict_lable.setGeometry(QRect(328, 240, 329, 27*4))
        # self.predict_lable.setWordWrap(True)
        # self.predict_lable.setAlignment(QtCore.Qt.AlignTop)

        self.predict_lable = QLabel(self)
        # self.predict_lable.setFixedSize(20, 20)
        # self.predict_lable.setWeight(18)
    ...
    def get_frame(self):
        while self.open:
            ...
            if not flag:
                ...
                # self.init_pic_label("没有检测到摄像头，请检查是否插入")
                # self.label = None
                # self.close_camera()
                # self.label=QLabel()
                break
