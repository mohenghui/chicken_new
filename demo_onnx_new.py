...
import qdarkstyle
# 打包命令 #nuitka --standalone --show-progress --show-memory --enable-plugin=pyqt5 --output-dir=out demo_onnx.py
# 运行该项目 demo_onnx.py
# main_gobal_w=0
# main_gobal_w=0
...
class ChildWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.child_w = self.main_w//4
        self.child_h = self.main_h//1.5
        self.resize(self.child_w, self.child_h)
        self.cb = QComboBox(self)
        # self.cb.resize(120, 40)
        self.cb.setMinimumSize(self.main_w//16, self.main_h//27)
        self.cb.move(130, 20)
...
class MainWindow(QMainWindow):
    ...
    def predict(self, img):
        ...
        input_data, raw_data = self.preprocess(img)
        results = self.model.run(
            None, {self.model.get_inputs()[0].name: input_data})

        self.predict_index = np.argmax(results)
        pred_label = self.classes[self.predict_index]
        confident = results
        conf = round(float(confident[0][0][self.predict_index]*100), 2)
    ...
    def init_pic_label(self, text):
        ...
        self.classes = ["母鸡", "公鸡", "其他"]
        self.predict_label_height = self.win_height//8
        # self.predict_lable.setGeometry(QRect(150, 50, 900, 27*6))
        self.predict_lable.setStyleSheet("font-size: {}pt".format(pointsize*90/72*1.2))
        self.predict_lable.setMinimumSize(
            self.win_width, self.predict_label_height)
        ...
        self.right_layout.addWidget(self.predict_lable, 0, 0)

        self.label = QLabel(self)
        self.label.setText("  {}".format(text))
        self.label.setStyleSheet("font-size: {}pt".format(pointsize*90/72*2))
        self.label.setAlignment(Qt.AlignCenter)
        # self.imageWindow.setMinimumSize(self.imageWindowWidth,self.imageWindowHeight)
        # self.label.setFixedSize(self.win_width//2, self.win_height)
        # self.label.setFixedSize(self.win_width//1, self.win_height)
        self.label.setMinimumSize(
            self.win_width, self.win_height-self.predict_label_height)
        # self.label.move(500, 300)
        self.right_layout.addWidget(self.label, 1, 0)
    ...
    def initUI(self):
        ...
        self.win_height = int(self.screenheight * 0.7)
        self.win_width = int(self.screenwidth * 0.7)
        # print(self.win_height,self.win_width)
        self.resize(self.win_width, self.win_height)
        # self.resize(1920, 1080)
        self.main_layout = QGridLayout()
        margin_size = self.win_width//90
        ...
        self.right_layout.setContentsMargins(
            self.win_width//18, self.win_height//25, self.win_width//18, self.win_height//25)  # 左上右下
        ...
        self.child = ChildWindow(self.win_width, self.win_height)
        self.button_width = self.win_width*0.25
        self.button_height = self.win_height/9*0.8
        self.button1 = QPushButton("从文件夹中选择", self,
                                   objectName="RedButton")
        # self.button1.setMinimumSize(self.button_height,self.button_width)
        self.button1.setMinimumSize(self.button_width, self.button_height)
        # self.bind_trigger()
        self.init_pic_label("显示图片")
        self.button1.clicked.connect(self.open_image)
        # self.button1.move(10, 40)
        self.left_layout.addWidget(self.button1, 0, 0)
        ...
        # self.button_c_open_g.setMinimumSize(self.button_height,self.button_width)
        # self.button_c_open_g.setMinimumSize(
        #     self.button_width, self.button_height)
        # self.left_layout.addWidget(self.button_c_open_g, 1, 0)
        ...
        # self.button_c_open.setMinimumSize(self.button_height,self.button_width)
        self.button_c_open.setMinimumSize(
            self.button_width, self.button_height)
        self.left_layout.addWidget(self.button_c_open, 2, 0)
        ...
        # self.button_c_close.setMinimumSize(self.button_height,self.button_width)
        self.button_c_close.setMinimumSize(
            self.button_width, self.button_height)
        # self.button_c_close.move(10, 276)
        self.left_layout.addWidget(self.button_c_close, 3, 0)
        ...
        self.left_layout.addWidget(self.button_s_predict, 4, 0)
        # self.button_s_predict.setMinimumSize(self.button_height,self.button_width)
        self.button_s_predict.setMinimumSize(
            self.button_width, self.button_height)
        ...
        # self.button_c_predict.setMinimumSize(self.button_height,self.button_width)
        self.button_c_predict.setMinimumSize(
            self.button_width, self.button_height)
        self.left_layout.addWidget(self.button_c_predict, 5, 0)

        # self.ser_layout=QGridLayout()

        self.device_button = QPushButton("搜索列表", self,
                                         objectName="RedButton")

        self.device_button.setMinimumSize(
            self.button_width, self.button_height)
        # self.bind_trigger()
        self.device_button.clicked.connect(self.enum_devices)
        self.left_layout.addWidget(self.device_button, 6, 0)
        # self.ser_layout.addWidget(self.device_button,0,0)

        self.ser_cb = QComboBox(self)
        self.ser_cb.setMinimumSize(self.button_width, self.button_height)
        # self.ser_cb.setAlignment(Qt.AlignCenter)
        self.ser_cb.setEditable(True)
        self.ser_cb.lineEdit().setAlignment(Qt.AlignCenter)
        # self.ser_cb.setEditable(False)
        self.ser_cb.currentTextChanged.connect(self.get_ser_device)
        self.left_layout.addWidget(self.ser_cb, 7, 0)
        # self.ser_layout.addWidget(self.ser_cb,1,0)

        # self.serial_lable = QLabel(self)
        # self.serial_lable.setMinimumSize(self.button_width,self.button_height)
        # self.serial_lable.setWordWrap(True)
        # # self.serial_lable.setStyleSheet("font-size: 27pt")
        # self.left_layout.addWidget(self.serial_lable,8,0)
        ...
        # self.listen_open_button.setMinimumSize(self.button_height,self.button_width)
        self.listen_open_button.setMinimumSize(
            self.button_width, self.button_height)
        self.left_layout.addWidget(self.listen_open_button, 8, 0)

        self.listen_close_button = QPushButton('停止监听', self,
                                               objectName="RedButton")
        self.listen_close_button.clicked.connect(self.close_listen)
        # self.listen_close_button.setMinimumSize(self.button_height,self.button_width)
        self.listen_close_button.setMinimumSize(
            self.button_width, self.button_height)
        # self.listen_close_button.move(10,600)
        self.left_layout.addWidget(self.listen_close_button, 9, 0)

        # self.main_ser=

        # self.left_layout.setRowMinimumHeight(self.button_width,self.button_height)
        # self.left_layout.setRowMinimumHeight(self.button_width,self.button_height)
        # self.left_layout.setColumnMinimumWidth
        ...
        # self.main_layout.setColumnMinimumWidth(0,self.button_width)
    ...
    def listen_sign(self):
        ...
        while self.listen_open:
            ...
            if self.ser:
                time.sleep(0.5)
                success_bytes = self.ser.write(bytes.fromhex(self.NO1_collect))
    ...
    def open_camera_g(self):
        ...
        self.g_open = True
        # self.get_frame_g()
        self.t2 = threading.Thread(target=self.get_frame_g)
        self.t2.start()
        # print ("Save Image succeed!")

    def get_frame_g(self):
        while self.g_open:
            # print("工业摄像机进来了")
    ...
    #     pass
        showImage = QtGui.QImage(
            self.img.data, self.img.shape[1], self.img.shape[0], QtGui.QImage.Format_RGB888)
        self.label.setPixmap(QtGui.QPixmap.fromImage(showImage))
    # def get_frame_single(self):
    ...
    def closeEvent(self, event):
        ...
        if reply == QMessageBox.Yes:
            event.accept()
            if self.open or self.g_open or self.listen_open or self.show_timeout_open:
                self.close_all()
